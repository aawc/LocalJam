...
- **Visual Change Verification & Artifact Capture (Before and After):**
  ...
  - **Capture Discipline:** Prior to altering UI code, execute `npm run capture -- --feature=<feature> --phase=before` to capture the baseline ("before") state. After applying modifications, execute `npm run capture -- --feature=<feature> --phase=after` under identical conditions. Both states must be preserved in the repository to provide verifiable visual regression evidence.
