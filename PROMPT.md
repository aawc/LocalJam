...
- **Capture Discipline:** Prior to altering UI code, capture baseline ("before") state of target component/viewport; capture modified ("after") state under identical conditions (viewport, theme, data fixtures). Both states must be preserved for regression validation.
...
- **Deterministic Naming:** Artifact filenames must adhere strictly to `<scope>-<state>-<viewport>-before.png` and `<scope>-<state>-<viewport>-after.png` (for example, `stage-playing-desktop-1280x800-before.png`).
...
- **Mandatory Commit Description Citation:** Any commit introducing or modifying visual UI elements must explicitly cite or link the before and after visual artifacts in the commit description using repository-relative paths (for example, `./docs/artifacts/visual/<feature>/<scope>-<state>-<viewport>-before.png` and `-after.png`). If visual capture was deferred due to a headless environment, the commit description must explicitly record `[VISUAL CAPTURE: DEFERRED (HEADLESS ENVIRONMENT)]` along with the specific technical rationale.
